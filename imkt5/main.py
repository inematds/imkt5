"""imkt5 main — bootstrap completo do Gateway.
...
from imkt5.capabilities.registry import CapabilityRegistry
from imkt5.gateway.api import create_app
from imkt5.gateway.http_dispatcher import HttpDispatcher
from imkt5.gateway.jobs_store import JobsStore
from imkt5.recipes.approvals import CompositeApprovalGate
from imkt5.recipes.loader import load_recipes_from_dir
from imkt5.recipes.runner import RecipeRunner
from imkt5.tools.run_recipe import StaticRecipeCatalog
from imkt5.types.approvals import ApprovalDecision
from imkt5.types.jobs import Job
...
class _AutoReviewGate:
    ...
    async def evaluate(
        ...
        from imkt5.types.jobs import Job
        import uuid
        from imkt5.capabilities.matcher import select_worker
...
def main() -> None:
    ...
    # Logging: INFO em loggers internos do imkt5
    ...
    for n in ("imkt5.dispatcher", "imkt5.delivery", "imkt5.telegram",
              "imkt5.agent", "imkt5.approvals", "imkt5.workers.llm"):
        _logging.getLogger(n).setLevel(_logging.INFO)

    workers_yaml = os.environ.get("IMKT5_WORKERS_YAML", "config/workers.yaml")
    recipes_dir = os.environ.get("IMKT5_RECIPES_DIR", "recipes")
    ...
    from imkt5.db.pool import get_pool
    db = get_pool()
    jobs_store: Any
    if db is not None:
        from imkt5.db.jobs_repo import PostgresJobsStore
    ...
    from imkt5.gateway.delivery import ChannelNotifier
    ...
    runs_repo = None
    if db is not None:
        from imkt5.db.runs_repo import RunsRepo
    ...
    from imkt5.recipes.approvals.telegram_gates import UserApprovalGate, HumanReviewerGate
    ...
    from imkt5.agent import AgentLoop, ContextBuilder
    from imkt5.agent.loop import AgentConfig
    from imkt5.memory.store import MemoryStore
    from imkt5.providers.ollama import OllamaProvider
    from imkt5.providers.openrouter import OpenRouterProvider
    from imkt5.tools.dispatch_job import DispatchJobTool
    from imkt5.tools.registry import ToolRegistry
    from imkt5.tools.run_recipe import RunRecipeTool
    from imkt5.config import load as _load_cfg

    memory = MemoryStore(_load_cfg().memory.db_path)

    provider_name = os.environ.get("IMKT5_AGENT_PROVIDER", "ollama")
    if provider_name == "openrouter":
        provider = OpenRouterProvider()
        agent_model = os.environ.get("IMKT5_AGENT_MODEL", _load_cfg().llm.openrouter.default_model)
    else:
        provider = OllamaProvider()
        agent_model = os.environ.get("IMKT5_AGENT_MODEL", _load_cfg().llm.ollama.router_model)
    ...
    tenancy_repo = None
    if db is not None:
        from imkt5.db.tenancy_repo import TenancyRepo
    ...
    from imkt5.channels.telegram import TelegramChannel, TelegramIdentity
    ...
    async def _startup() -> None:
        ...
        try:
            from imkt5.gateway.audit import AuditSink as _AuditSink
