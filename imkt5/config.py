...
ENV_MAP: dict[str, tuple[str, ...]] = {
    ...
    # dispatcher
    "IMKT5_DISPATCH_TIMEOUT": ("dispatcher", "request_timeout_seconds"),
    "IMKT5_DISPATCH_RETRY_SECONDS": ("dispatcher", "retry_when_saturated_seconds"),
    "IMKT5_DISPATCH_MAX_PENDING": ("dispatcher", "max_pending_jobs"),
    "IMKT5_DISPATCH_MAX_RETRIES": ("dispatcher", "max_select_retries"),
    # registry
    "IMKT5_PROBE_TIMEOUT": ("capability_registry", "probe_timeout_seconds"),
    # jobs store
    "IMKT5_JOBS_HISTORY": ("jobs_store", "max_history"),
    # approvals
    "IMKT5_APPROVAL_TIMEOUT": ("approvals", "default_timeout_seconds"),
    # storage
    "IMKT5_ARTIFACT_ROOT": ("storage", "artifact_root"),
    # memory
    ...
    "IMKT5_INEMAIMG_ADAPTER_PORT": ("workers", "inemaimg_adapter", "port"),
    ...
    "IMKT5_INEMAVOX_ADAPTER_PORT": ("workers", "inemavox_adapter", "port"),
...
# ── Acesso "mágico" via módulo (imkt5.config.settings.gateway.port) ─
# Permite `from imkt5.config import settings` como atalho equivalente a
# `from imkt5.config import settings as _get; settings = _get()`.
