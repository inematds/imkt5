...
"""
from imkt5.gateway._media_modal import MEDIA_MODAL_HTML as _MEDIA_MODAL_HTML
from imkt5.gateway._modal_close import UNIVERSAL_MODAL_HTML as _UNIVERSAL_MODAL_HTML
...
<header style="display:flex;align-items:center;gap:20px;">
  ...
  <nav style="display:flex;gap:4px;font-size:13px;">
    ...
    <a href="/chat-ui"    style="color:var(--fg2);text-decoration:none;padding:6px 12px;border-radius:6px;">Chat</a>
...
<script>
  function setUserToken() {
    const cur = localStorage.getItem('imkt5_user_token') || '';
    const t = prompt('Token de usuário (Bearer):', cur);
    if (t !== null) { localStorage.setItem('imkt5_user_token', t.trim()); location.reload(); }
  }
  (function updateTok() {
    const t = localStorage.getItem('imkt5_user_token') || localStorage.getItem('imkt5_admin_token') || '';
...
async function jpost(p, b) {
  ...
  if (r.status === 401) {
    if (confirm('Token ausente/inválido. Configurar agora?')) {
      const t = prompt('Token:', localStorage.getItem('imkt5_user_token') || '');
      if (t) { localStorage.setItem('imkt5_user_token', t.trim()); location.reload(); }
    }
    return null;
  }
  return r.json();
