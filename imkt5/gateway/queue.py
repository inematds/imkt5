...
class JobQueue:
    ...
    def _queue_name(job: Job) -> str:
        if job.worker_type:
            return f"imkt5:jobs:worker:{job.worker_type}"
        return f"imkt5:jobs:capability:{job.required_capability}"
    ...
    async def dequeue(
        ...
        for w in worker_types or []:
            names.append(f"imkt5:jobs:worker:{w}")
        for c in capabilities or []:
            names.append(f"imkt5:jobs:capability:{c}")
        if not names:
            return None
...
def _deserialize_job(d: dict[str, Any]) -> Job:
    from imkt5.types.jobs import JobPriority as _P
