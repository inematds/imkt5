...
// ── token ─────────────────────────────────────────────────────
function getToken() {
  return localStorage.getItem('imkt5_user_token')
      || localStorage.getItem('imkt5_admin_token') || '';
}
function setToken() {
  const t = prompt('Token (Bearer):', getToken());
  if (t !== null) {
    localStorage.setItem('imkt5_user_token', t.trim());
    location.reload();
  }
}
...
function getSession() {
  let s = localStorage.getItem('imkt5_chat_session');
  if (!s) {
    s = 'chat-' + shortId();
    localStorage.setItem('imkt5_chat_session', s);
  }
  return s;
}
function newSession() {
  ...
  localStorage.setItem('imkt5_chat_session', s);
  localStorage.removeItem('imkt5_chat_history');
  location.reload();
}
...
function getHistory() {
  try { return JSON.parse(localStorage.getItem('imkt5_chat_history') || '[]'); }
  catch { return []; }
}
function saveHistory(h) {
  localStorage.setItem('imkt5_chat_history', JSON.stringify(h.slice(-200)));
}
...
function loadHistory() {
  ...
  if (h.length === 0) {
    ...
    welcome.innerHTML = `
      <div class="bubble">
        👋 Oi! Sou o agente do imkt5. Posso <b>responder diretamente</b>,
