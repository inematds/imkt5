...
<div class="topbar">
  ...
  <nav style="display:flex;gap:4px;font-size:13px;">
    ...
    <a href="/admin"      style="color:#1f6feb;text-decoration:none;padding:6px 12px;border-radius:6px;background:#21262d;font-weight:600;">Admin</a>
  </nav>
  <div style="width:1px;height:20px;background:#30363d;margin:0 8px;"></div>
  <div class="tabs">
...
// ── token + fetch wrapper ─────────────────────────────────────
function getToken() {
  return localStorage.getItem('imkt5_admin_token') || '';
}
function setToken() {
  const cur = getToken();
  const t = prompt('Token admin (Bearer):', cur);
  if (t !== null) {
    localStorage.setItem('imkt5_admin_token', t.trim());
    updateAuthStatus();
    location.reload();
  }
}
