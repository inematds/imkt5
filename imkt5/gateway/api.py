...
from imkt5.gateway.admin_ui import ADMIN_HTML
from imkt5.gateway.recipes_ui import RECIPES_UI_HTML
from imkt5.gateway.runs_ui import RUNS_UI_HTML
from imkt5.gateway.workers_ui import WORKERS_UI_HTML
from imkt5.gateway.chat_ui import CHAT_UI_HTML
from imkt5.gateway.audit import AuditSink, make_audit_middleware
from imkt5.gateway.auth import Principal, current_principal
from imkt5.gateway.jobs_store import JobsStore
from imkt5.gateway.web_ui import UI_HTML
...
from imkt5.types.jobs import Job, JobPriority
...
def create_app(
    *,
    ...
    app = FastAPI(title="imkt5 Gateway", version="0.0.1")
    ...
    async def _require_user(request: Any, tenant_id: str | None = None) -> Principal:
        ...
        tenant. Modo anônimo é aceito quando IMKT5_AUTH_REQUIRED != 1 — isso
        mantém o gateway utilizável em dev.
        """
        p = await current_principal(request, request.headers.get("authorization"))
        # Em dev (sem IMKT5_AUTH_REQUIRED=1), anon pode tudo — é o padrão atual.
        if p.roles == ("anon",):
            import os as _os
            if _os.environ.get("IMKT5_AUTH_REQUIRED", "0") != "1":
                return p
    ...
    async def list_runs(
        ...
        if repo is not None:
            try:
                ...
                import logging
                logging.getLogger("imkt5.api").warning("runs_repo.list falhou: %s", exc)
        out = []
        for r in runs:
            stage_counts: dict[str, int] = {}
            for s in r.stages.values():
    ...
    async def rerun_from_stage(run_id: str, stage_id: str, request: Request) -> dict[str, Any]:
        ...
        from imkt5.recipes.runner import StageStatus as _St
    ...
    async def delete_run(run_id: str, request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            logging.getLogger("imkt5.api").warning(
    ...
    async def download_bundle(run_id: str) -> Any:
        ...
        def _resolve_local_path(url: str) -> Path | None:
            ...
            if url.startswith("/artifacts/"):
                root = os.environ.get("IMKT5_ARTIFACT_ROOT", str(ARTIFACT_ROOT))
    ...
    async def chat(req: ChatRequest, request: Request) -> dict[str, Any]:
        ...
        from imkt5.types.messages import IncomingMessage
    ...
    async def video_thumb(u: str, t: float = 2.0) -> FileResponse:
        ...
        cache_dir = Path("/tmp/imkt5-video-thumbs")
