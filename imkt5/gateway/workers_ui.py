...
<script>
function getToken() { return localStorage.getItem('imkt5_admin_token') || ''; }
function setToken() {
  const t = prompt('Token admin (Bearer):', getToken());
  if (t !== null) { localStorage.setItem('imkt5_admin_token', t.trim()); location.reload(); }
}
...
let statusFilter = '';
let workerSort = localStorage.getItem('imkt5_worker_sort') || 'name';  // name | usage | recent
function setSort(s) {
  workerSort = s;
  localStorage.setItem('imkt5_worker_sort', s);
