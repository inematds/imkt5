...
Tokens e seus papéis vêm de env `IMKT4_AUTH_TOKENS` (JSON) ou
`config/auth.yaml`. Em dev, se não há nada configurado, roda em modo
aberto (audit-only). Em prod, setar `IMKT4_AUTH_REQUIRED=1`.
...
Uso (FastAPI):
    from imkt4.gateway.auth import Principal, require
...
log = logging.getLogger("imkt4.auth")
...
def _load_tokens() -> dict[str, dict[str, Any]]:
    """Carrega tokens de env IMKT4_AUTH_TOKENS (JSON) ou arquivo.
    ...
    """
    raw = os.environ.get("IMKT4_AUTH_TOKENS")
    if raw:
        try:
            return json.loads(raw)
        except json.JSONDecodeError:
            log.warning("IMKT4_AUTH_TOKENS inválido — ignorando")

    path = os.environ.get("IMKT4_AUTH_TOKENS_FILE", "config/auth.json")
...
def _required() -> bool:
    return os.environ.get("IMKT4_AUTH_REQUIRED", "0") == "1"
