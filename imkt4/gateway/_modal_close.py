...
/* Classe .imkt4-managed-modal (adicionada automaticamente):
   garante relative no card interno pra X posicionar ok */
.imkt4-managed-modal > div,
.imkt4-managed-modal > .inner,
.imkt4-managed-modal > section,
.imkt4-managed-modal > form {
  position: relative;
}
</style>
...
(function(){
  ...
  function injectCloseX(el) {
    if (el.querySelector(':scope > .imkt4-modal-x')) return;  // já tem
    const btn = document.createElement('button');
    btn.className = 'imkt4-modal-x';
    ...
    el.classList.add('imkt4-managed-modal');
    el.appendChild(btn);
  }
